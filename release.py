...
def main() -> int:
    ...
    bin_path = go_bin_dir / "revproxyhashry"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        bin_package_dir = pathlib.Path(tmp_dir) / "revproxyhashry-{}-linux-x64".format(version)

        target = bin_package_dir / "bin/revproxyhashry".format(version)
        target.parent.mkdir(parents=True)

        shutil.copy(bin_path.as_posix(), target.as_posix())

        tar_path = bin_package_dir.parent / "revproxyhashry-{}-linux-x64.tar.gz".format(version)

        subprocess.check_call([
            "tar", "-czf", tar_path.as_posix(), "revproxyhashry-{}-linux-x64".format(version)],
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        deb_package_dir = pathlib.Path(tmp_dir) / "revproxyhashry_{}_amd64".format(version)

        target = deb_package_dir / "usr/bin/revproxyhashry".format(version)
        target.parent.mkdir(parents=True)
        shutil.copy(bin_path.as_posix(), target.as_posix())
        ...
        control_pth.write_text("\n".join([
            "Package: revproxyhashry",
            "Version: {}".format(version),
            "Maintainer: Marko Ristin (marko@parquery.com)",
            "Architecture: amd64",
            "Description: revproxyhashry generates bcrypt hashes of passwords to be used with the revproxyry.",
            ""]))
        ...
        deb_pth = deb_package_dir.parent / "revproxyhashry_{}_amd64.deb".format(version)
